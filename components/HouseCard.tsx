...
function getMonthRange() {
  const now = new Date();
  return {
    start: Timestamp.fromDate(new Date(now.getFullYear(), now.getMonth(), 1)),
    end: Timestamp.fromDate(new Date(now.getFullYear(), now.getMonth() + 1, 1)),
  };
}
...
export default function HouseCard({
  userId, buildingId, buildingName, house, onDelete, settings
}: {
  userId: string;
  buildingId: string;
  buildingName: string;
  ...
  useEffect(() => {
    ...
    const q = query(
      collection(db, `users/${userId}/buildings/${buildingId}/houses/${house.id}/payments`),
      where('date', '>=', start),
      where('date', '<', end)
    );
  ...
  const handleAddPayment = async () => {
    ...
    setSaving(true);
    try {
      await addDoc(collection(db, `users/${userId}/buildings/${buildingId}/houses/${house.id}/payments`), {
      ...
      if (settings.showReceipts) setShowReceiptModal(true);
  ...
  const handleEditSave = async () => {
    setSaving(true);
    try {
      await updateDoc(doc(db, `users/${userId}/buildings/${buildingId}/houses/${house.id}`), {
  ...
  const openDetails = () => {
    if (settings.showTenantDetails) setShowDetailsModal(true);
  };

  return (
    <>
      <div
        style={{ background: 'var(--paper-card)', borderRadius: 'var(--radius)', border: `1.5px solid ${fullyPaid ? '#86efac' : 'var(--border)'}`, boxShadow: 'var(--shadow-sm)', overflow: 'hidden', transition: 'box-shadow 0.18s' }}
      ...
      >
        ...
        <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8, background: fullyPaid ? 'rgba(34,197,94,0.05)' : 'var(--paper-elevated)' }}>
          <div style={{ minWidth: 0 }}>
            ...
            <button onClick={openDetails} style={{ background: 'none', border: 'none', padding: 0, cursor: settings.showTenantDetails ? 'pointer' : 'default', textAlign: 'left' }}>
              <p style={{ color: noTenant ? 'var(--ink-muted)' : 'var(--accent)', fontSize: '0.88rem', marginTop: 2, textDecoration: (settings.showTenantDetails && !noTenant) ? 'underline' : 'none', textDecorationStyle: 'dotted' }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexShrink: 0, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
            ...
            <button
              ...
              style={{ background: 'var(--paper-warm)', color: 'var(--ink-soft)', border: 'none', borderRadius: 8, padding: '5px 10px', fontSize: '0.78rem', cursor: 'pointer' }}
            >✏️ Edit</button>
            <button
              onClick={onDelete}
              style={{ background: 'var(--paper-warm)', color: 'var(--ink-muted)', border: 'none', borderRadius: 8, padding: '5px 10px', fontSize: '0.78rem', cursor: 'pointer' }}
      ...
      {showDetailsModal && settings.showTenantDetails && (
        <Modal title={`Tenant Details — ${house.houseNumber}`} onClose={() => setShowDetailsModal(false)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {[
              { label: 'Unit', val: house.houseNumber },
              { label: 'Tenant Name', val: house.tenantName || '—' },
              { label: 'Phone', val: house.phoneNumber || '—' },
              { label: 'Monthly Rent', val: house.monthlyRent ? `PKR ${Number(house.monthlyRent).toLocaleString()}` : 'Not set' },
              { label: 'Paid This Month', val: `PKR ${totalPaid.toLocaleString()}` },
              { label: 'Pending', val: pending > 0 ? `PKR ${pending.toLocaleString()}` : 'Fully paid ✅' },
            ].map(({ label, val }) => (
            ...
            <div style={{ padding: '12px', background: 'var(--amber-soft)', borderRadius: 'var(--radius-sm)', fontSize: '0.82rem', color: 'var(--amber)' }}>
              📄 Lease contract upload — coming soon.
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
              {settings.showWhatsApp && house.phoneNumber && pending > 0 && (
                <Btn variant="ghost" onClick={openWhatsApp} fullWidth>📱 WhatsApp</Btn>
              )}
      ...
      {/* Payment History */}
      {showHistory && settings.showPastRecords && (
